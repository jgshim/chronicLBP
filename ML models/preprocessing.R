...
set.seed(42)
training.samples <- createDataPartition(hn1415$LBP, p = 0.7, list = FALSE)
train  <- hn1415[training.samples, ]
test <- hn1415[-training.samples, ]

table(train$LBP)
table(test$LBP)

###########################################
## !!! Training Data scaling & 범주특성의 변환 ## ----
###########################################
...
train_MinMaxScale <- preProcess(train_num, method=c("range"))
print(train_MinMaxScale)
train_minmax <- predict(train_MinMaxScale, train_num)
head(train_minmax)

###########################################
## !!! Test Data scaling & 범주특성의 변환 ## ----
###########################################
...
test_minmax <- predict(train_MinMaxScale, test_num)
head(test_minmax)
...
write.csv(train, file="train_p.csv", row.names = TRUE)
write.csv(test, file="test_p.csv", row.names = TRUE)
